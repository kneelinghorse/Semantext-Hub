#!/usr/bin/env node

/**
 * CI Performance Budget Guard
 *
 * Reads `.artifacts/perf/summary.json` (or a supplied path) generated by
 * `node cli/index.js perf:status --format json` and enforces simple
 * P95 latency ceilings for discovery and MCP services.
 *
 * Budgets are configurable via CLI flags or environment variables:
 *   PERF_BUDGET_DISCOVERY_P95 (ms)
 *   PERF_BUDGET_MCP_P95 (ms)
 *
 * The script exits with code 1 if the budgets are exceeded or if the
 * metrics payload is missing the required fields.
 */

import fs from 'fs/promises';
import path from 'path';
import process from 'process';
import { fileURLToPath } from 'url';

const ENV_DISCOVERY_P95 = process.env.PERF_BUDGET_DISCOVERY_P95;
const ENV_MCP_P95 = process.env.PERF_BUDGET_MCP_P95;
const ENV_REGISTRY_GET_P95 = process.env.PERF_BUDGET_REGISTRY_GET_P95;
const ENV_RESOLVE_P95 = process.env.PERF_BUDGET_RESOLVE_P95;

const DEFAULT_DISCOVERY_P95 = Number.parseInt(ENV_DISCOVERY_P95 ?? '1000', 10);
const DEFAULT_MCP_P95 = Number.parseInt(ENV_MCP_P95 ?? '3000', 10);
const DEFAULT_REGISTRY_GET_P95 = Number.parseInt(ENV_REGISTRY_GET_P95 ?? '150', 10);
const DEFAULT_RESOLVE_P95 = Number.parseInt(ENV_RESOLVE_P95 ?? '300', 10);

function resolveFromCwd(targetPath) {
  return path.resolve(process.cwd(), targetPath);
}

function parseArgs() {
  const args = process.argv.slice(2);
  const options = {
    file: '.artifacts/perf/summary.json',
    discoveryBudget: DEFAULT_DISCOVERY_P95,
    mcpBudget: DEFAULT_MCP_P95,
    registryGetBudget: DEFAULT_REGISTRY_GET_P95,
    resolveBudget: DEFAULT_RESOLVE_P95,
    registryGetBudgetExplicit: ENV_REGISTRY_GET_P95 !== undefined,
    resolveBudgetExplicit: ENV_RESOLVE_P95 !== undefined
  };

  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i];

    if ((arg === '--file' || arg === '-f') && args[i + 1]) {
      options.file = args[i + 1];
      i += 1;
    } else if (arg === '--discovery-p95' && args[i + 1]) {
      options.discoveryBudget = Number.parseFloat(args[i + 1]);
      i += 1;
    } else if (arg === '--mcp-p95' && args[i + 1]) {
      options.mcpBudget = Number.parseFloat(args[i + 1]);
      i += 1;
    } else if (arg === '--registry-get-p95' && args[i + 1]) {
      options.registryGetBudget = Number.parseFloat(args[i + 1]);
      options.registryGetBudgetExplicit = true;
      i += 1;
    } else if (arg === '--resolve-p95' && args[i + 1]) {
      options.resolveBudget = Number.parseFloat(args[i + 1]);
      options.resolveBudgetExplicit = true;
      i += 1;
    } else if (arg === '--help' || arg === '-h') {
      printHelp();
      process.exit(0);
    }
  }

  return options;
}

function printHelp() {
  const scriptPath = path.relative(
    process.cwd(),
    fileURLToPath(import.meta.url)
  );
  console.log(`
Usage: node ${scriptPath} [options]

Options:
  --file, -f <path>         Path to perf summary JSON (default: .artifacts/perf/summary.json)
  --discovery-p95 <ms>      Discovery P95 budget in milliseconds
  --mcp-p95 <ms>            MCP P95 budget in milliseconds
  --registry-get-p95 <ms>   Registry GET P95 budget in milliseconds
  --resolve-p95 <ms>        Resolve P95 budget in milliseconds
  --help, -h                Show this help message

Environment overrides:
  PERF_BUDGET_DISCOVERY_P95   (default: ${DEFAULT_DISCOVERY_P95})
  PERF_BUDGET_MCP_P95         (default: ${DEFAULT_MCP_P95})
  PERF_BUDGET_REGISTRY_GET_P95 (default: ${DEFAULT_REGISTRY_GET_P95})
  PERF_BUDGET_RESOLVE_P95     (default: ${DEFAULT_RESOLVE_P95})
`);
}

function calculateStats(results) {
  const durations = results
    .map((r) => {
      const candidate =
        r.ms ??
        r.duration ??
        r.latency ??
        r.time ??
        r.responseTime ??
        null;
      return typeof candidate === 'number' ? candidate : Number.parseFloat(candidate);
    })
    .filter((value) => Number.isFinite(value))
    .sort((a, b) => a - b);

  const count = durations.length;
  if (count === 0) {
    return { count: 0, avg: null, p95: null };
  }

  const sum = durations.reduce((acc, value) => acc + value, 0);
  const avg = sum / count;
  const p95Index = Math.min(count - 1, Math.ceil(count * 0.95) - 1);
  const p95 = durations[p95Index];

  return { count, avg, p95 };
}

function validateMetrics(data) {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid perf data: expected JSON object.');
  }

  // Handle legacy perf.json format
  if (data && (data.discovery || data.mcp)) {
    return {
      discoveryP95: data.discovery?.p95 ?? null,
      discoveryCount: data.discovery?.total ?? null,
      mcpP95: data.mcp?.p95 ?? null,
      mcpCount: data.mcp?.total ?? null,
      registryGetP95: null,
      registryGetCount: null,
      resolveP95: null,
      resolveCount: null
    };
  }

  // Handle JSONL format from smoke test
  if (Array.isArray(data)) {
    const registryGetResults = data.filter((entry) => {
      if (entry.route === 'registry_get') {
        return true;
      }
      if (entry.method === 'GET' && typeof entry.path === 'string') {
        return (
          entry.path === '/registry' ||
          entry.path.startsWith('/registry?') ||
          entry.path.startsWith('/v1/registry/')
        );
      }
      return false;
    });

    const resolveResults = data.filter((entry) => {
      if (entry.route === 'resolve') {
        return true;
      }
      if (entry.method === 'GET' && typeof entry.path === 'string') {
        return entry.path.startsWith('/resolve') || entry.path.startsWith('/v1/resolve');
      }
      return false;
    });

    const registryGetStats = calculateStats(registryGetResults);
    const resolveStats = calculateStats(resolveResults);

    return {
      discoveryP95: null,
      discoveryCount: null,
      mcpP95: null,
      mcpCount: null,
      registryGetP95: registryGetStats.p95,
      registryGetCount: registryGetStats.count,
      resolveP95: resolveStats.p95,
      resolveCount: resolveStats.count
    };
  }

  throw new Error('Invalid perf data format: expected perf summary JSON or JSONL array.');
}

async function main() {
  const options = parseArgs();
  const perfFile = resolveFromCwd(options.file);

  // Check if file exists
  let stats;
  try {
    stats = await fs.stat(perfFile);
  } catch (error) {
    throw new Error(
      `❌ Performance metrics file not found: ${perfFile}\n` +
      `   Ensure tests generate telemetry logs before running budget checks.\n` +
      `   Expected: npm run test:fast or npm run test:performance to create artifacts.`
    );
  }

  // Check if file is stale (older than 1 hour)
  const fileAge = Date.now() - stats.mtimeMs;
  const MAX_AGE_MS = 60 * 60 * 1000; // 1 hour
  if (fileAge > MAX_AGE_MS) {
    const ageMinutes = Math.floor(fileAge / 60000);
    throw new Error(
      `❌ Performance metrics file is stale (${ageMinutes} minutes old): ${perfFile}\n` +
      `   Re-run tests to generate fresh metrics.`
    );
  }

  let metricsRaw;
  try {
    metricsRaw = await fs.readFile(perfFile, 'utf8');
  } catch (error) {
    throw new Error(
      `Unable to read performance metrics at ${perfFile}: ${error.message}`
    );
  }

  // Check if file is empty
  if (!metricsRaw || metricsRaw.trim().length === 0) {
    throw new Error(
      `❌ Performance metrics file is empty: ${perfFile}\n` +
      `   Tests may have failed to generate telemetry data.`
    );
  }

  let data;
  try {
    // Try to parse as JSON first
    data = JSON.parse(metricsRaw);
  } catch (error) {
    // If JSON parsing fails, try parsing as JSONL
    try {
      const lines = metricsRaw.trim().split('\n').filter(line => line.trim());
      data = lines.map(line => JSON.parse(line));
    } catch (jsonlError) {
      throw new Error(
        `Invalid JSON/JSONL in ${perfFile}: ${error.message}`
      );
    }
  }

  const {
    discoveryP95,
    discoveryCount,
    mcpP95,
    mcpCount,
    registryGetP95,
    registryGetCount,
    resolveP95,
    resolveCount
  } = validateMetrics(data);

  const missingMetrics = [];
  const missingOptionalMetrics = [];
  const checkRequired = (label, value, count, { optional = false } = {}) => {
    if (!Number.isFinite(value)) {
      if (optional) {
        missingOptionalMetrics.push(`${label} (missing)`);
      } else {
        missingMetrics.push(`${label} (missing)`);
      }
      return;
    }
    if (typeof count === 'number' && count <= 0) {
      if (optional) {
        missingOptionalMetrics.push(`${label} (no samples)`);
      } else {
        missingMetrics.push(`${label} (no samples)`);
      }
    }
  };

  if (Number.isFinite(options.discoveryBudget)) {
    checkRequired('discovery.p95', discoveryP95, discoveryCount);
  }
  if (Number.isFinite(options.mcpBudget)) {
    checkRequired('mcp.p95', mcpP95, mcpCount);
  }
  if (Number.isFinite(options.registryGetBudget)) {
    const optional =
      !options.registryGetBudgetExplicit &&
      registryGetCount == null &&
      !Number.isFinite(registryGetP95);
    checkRequired('registry.get.p95', registryGetP95, registryGetCount, { optional });
  }
  if (Number.isFinite(options.resolveBudget)) {
    const optional =
      !options.resolveBudgetExplicit &&
      resolveCount == null &&
      !Number.isFinite(resolveP95);
    checkRequired('resolve.p95', resolveP95, resolveCount, { optional });
  }

  if (missingMetrics.length > 0) {
    throw new Error(
      `❌ Missing required performance metrics in ${perfFile}:\n` +
      missingMetrics.map((item) => `   - ${item}`).join('\n') +
      `\n   Ensure JSONL telemetry exists under artifacts/perf/ and re-run perf collection.\n` +
      '   See artifacts/perf/README.md for log format requirements.'
    );
  }

  if (missingOptionalMetrics.length > 0) {
    console.warn(
      [
        '⚠️ Optional metrics missing:',
        ...missingOptionalMetrics.map((item) => `   - ${item}`),
        '   Budgets not requested; skipping optional checks.'
      ].join('\n')
    );
  }

  const violations = [];

  // Legacy metrics
  if (Number.isFinite(discoveryP95) && discoveryP95 > options.discoveryBudget) {
    violations.push(
      `discovery.p95 ${discoveryP95.toFixed(2)}ms exceeds budget ${options.discoveryBudget}ms`
    );
  }

  if (Number.isFinite(mcpP95) && mcpP95 > options.mcpBudget) {
    violations.push(
      `mcp.p95 ${mcpP95.toFixed(2)}ms exceeds budget ${options.mcpBudget}ms`
    );
  }

  // Registry HTTP metrics
  if (Number.isFinite(registryGetP95) && registryGetP95 > options.registryGetBudget) {
    violations.push(
      `registry.get.p95 ${registryGetP95.toFixed(2)}ms exceeds budget ${options.registryGetBudget}ms`
    );
  }

  if (Number.isFinite(resolveP95) && resolveP95 > options.resolveBudget) {
    violations.push(
      `resolve.p95 ${resolveP95.toFixed(2)}ms exceeds budget ${options.resolveBudget}ms`
    );
  }

  if (violations.length > 0) {
    const message = [
      '❌ Performance budgets not met:',
      ...violations.map((item) => `  - ${item}`)
    ].join('\n');
    throw new Error(message);
  }

  const passedBudgets = [];
  
  if (Number.isFinite(discoveryP95)) {
    passedBudgets.push(`discovery.p95 ${discoveryP95.toFixed(2)}ms ≤ ${options.discoveryBudget}ms`);
  }
  if (Number.isFinite(mcpP95)) {
    passedBudgets.push(`mcp.p95 ${mcpP95.toFixed(2)}ms ≤ ${options.mcpBudget}ms`);
  }
  if (Number.isFinite(registryGetP95)) {
    passedBudgets.push(`registry.get.p95 ${registryGetP95.toFixed(2)}ms ≤ ${options.registryGetBudget}ms`);
  }
  if (Number.isFinite(resolveP95)) {
    passedBudgets.push(`resolve.p95 ${resolveP95.toFixed(2)}ms ≤ ${options.resolveBudget}ms`);
  }

  console.log(
    [
      '✅ Performance budgets met:',
      ...passedBudgets.map(budget => `  - ${budget}`)
    ].join('\n')
  );
}

main().catch((error) => {
  console.error(error.message || error);
  process.exit(1);
});
