...
const ADAPTER_NAME = '__ADAPTER_NAME__';
const ADAPTER_TYPE = '__ADAPTER_TYPE__';
const DEFAULT_CAPABILITIES = __CAPABILITIES_JSON__;
...
function summarizeApi(spec) {
  ...
  return {
    title: spec?.info?.title ?? '__ADAPTER_DISPLAY_NAME__',
...
function summarizeData(spec) {
  ...
  return {
    title: spec?.name ?? spec?.info?.title ?? '__ADAPTER_DISPLAY_NAME__',
...
function summarizeSpec(spec, type) {
  switch (type) {
    ...
    default:
      return {
        title: '__ADAPTER_DISPLAY_NAME__',
        version: null,
        itemsCount: 0,
      };
  }
...
export async function run(argv = process.argv.slice(2)) {
  ...
  const specPath = options.spec ?? '__SPEC_RELATIVE_PATH__';
  const outDir = options.out ?? '__OUTPUT_RELATIVE_PATH__';
  const capabilities =
