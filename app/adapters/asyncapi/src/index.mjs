...
const ADAPTER_NAME = 'asyncapi';
const ADAPTER_TYPE = 'event';
const DEFAULT_CAPABILITIES = [
  "adapter.event.discover",
  "adapter.event.normalize"
];
...
function summarizeApi(spec) {
  ...
  return {
    title: spec?.info?.title ?? 'AsyncAPI Reference',
...
function summarizeData(spec) {
  ...
  return {
    title: spec?.name ?? spec?.info?.title ?? 'AsyncAPI Reference',
...
export async function run(argv = process.argv.slice(2)) {
  ...
  const outDir = options.out ?? '../../artifacts/adapters/asyncapi';
  const capabilities =
