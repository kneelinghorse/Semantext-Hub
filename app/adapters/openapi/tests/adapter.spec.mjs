...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), 'openapi-'));
});
...
test('buildAdapter produces catalog artifacts', async () => {
  ...
  const specPath = join(__dirname, '..', 'fixtures', 'spec.json');
  ...
  assert.equal(catalog.adapter.name, 'openapi');
  assert.equal(catalog.adapter.type, 'api');
