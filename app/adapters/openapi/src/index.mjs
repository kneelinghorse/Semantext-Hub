...
const ADAPTER_NAME = 'openapi';
const ADAPTER_TYPE = 'api';
...
function summarizeApi(spec) {
  ...
  return {
    title: spec?.info?.title ?? 'OpenAPI Reference',
...
function summarizeData(spec) {
  ...
  return {
    title: spec?.name ?? spec?.info?.title ?? 'OpenAPI Reference',
...
export async function run(argv = process.argv.slice(2)) {
  ...
  const specPath = options.spec ?? './fixtures/spec.json';
  const outDir = options.out ?? '../../artifacts/adapters/openapi';
  const capabilities =
