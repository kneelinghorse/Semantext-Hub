...
import { EXIT_CODES, run } from '../../app/cli/perf-status.mjs';
import {
  evaluateBudgets,
  loadPerfLogEntries,
  resolvePerfLogFile,
...
describe('perf-status CLI', () => {
  ...
  it('summarizes metrics and returns EXIT_CODES.OK when budgets pass', async () => {
    ...
    const { path: logPath } = await resolvePerfLogFile({ root: tempRoot, sessionId });
    const entries = await loadPerfLogEntries(logPath);
