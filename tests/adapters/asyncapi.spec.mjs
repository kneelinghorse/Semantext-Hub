...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), 'adapter-asyncapi-'));
});
...
test('asyncapi adapter builds catalog', async () => {
  ...
  expect(catalog.adapter.name).toBe('asyncapi');
  expect(catalog.adapter.type).toBe('event');
