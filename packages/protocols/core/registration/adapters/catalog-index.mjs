...
import { isValidURN } from '../../graph/urn-utils.js';
...
class CatalogIndexAdapter {
  ...
  validateManifest(manifest) {
    ...
    if (!manifest) {
      ...
      const hasUrn = typeof urn === 'string' && urn.trim().length > 0;
      const genericUrnPattern = /^urn:[a-zA-Z0-9][a-zA-Z0-9.-]*(:[a-zA-Z0-9][a-zA-Z0-9._/-]*){2,}(?:@[A-Za-z0-9][A-Za-z0-9._-]*)?$/;
      const urnIsValid = hasUrn && (isValidURN(urn) || genericUrnPattern.test(urn));
  ...
  findByNamespace(namespace) {
    const result = this.catalogIndex.findByNamespace(namespace);
   ...
   */
  findByOwner(owner) {
    const result = this.catalogIndex.findByOwner(owner);
    return Array.isArray(result?.results) ? result.results : [];
  }
   ...
   */
  findByType(type) {
    const result = this.catalogIndex.findByType(type);
    return Array.isArray(result?.results) ? result.results : [];
  }
   ...
   */
  detectCycles() {
    const result = this.catalogIndex.detectCycles();
    const cycles = Array.isArray(result?.cycles) ? result.cycles : [];
    return {
      hasCycles: Boolean(result?.hasCycles ?? result?.hasCycle ?? (cycles.length > 0)),
      cycles
    };
  }
}
